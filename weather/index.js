...
module.exports = config => new Promise(resolve=>{
	fetch("http://api.wunderground.com/api/" + config.config.wuKey +  "/astronomy/conditions/alerts/q/NH/Amherst.json").then(x=>x.json()).then(obj=>{
		...
		resolve("It's " + obj.current_observation.icon + " today, and it feels like " + obj.current_observation.feelslike_string + (obj.current_observation.feelslike_string === obj.current_observation.temperature_string ?
		 ""
